...
- **Headline & political newsflow — scan explicitly; this is intraday, not a scheduled event.** Presidential/leader remarks that move markets in real time: **Trump White House comments, press-gaggle quotes and Truth Social posts**, tariff/trade announcements, sanctions, executive orders, central-bank jawboning, and any official "deal is close / we're going to attack" headline. These whipsaw the tape *during the session* and are easy to miss with an events-only sweep. *Concrete lesson: do not ever again miss the Trump newsflow that moved markets (e.g. Jun 10 — "we're going to be attacking them very hard" after floating a deal "in two or three days" the day before, which whipsawed oil and equities).* Run a dedicated search for "[leader] today markets/Truth Social/tariffs" every refresh, name the specific quote and the market reaction, and give the political read (incl. non-consensus).
...
**Positioning — how "crowded" is judged.** Positioning is whether the crowd is already on our side (no one left to push it our way) — assessed from (1) RSI on the absolute 30/70 convention, (2) **moving-average stretch and Bollinger Bands** — price stretched far above the 50/100/200-day MAs and riding the upper band = extended/crowded (fade-prone); hugging the lower band below the MAs = sellers exhausted (squeeze-prone); a Bollinger squeeze = a crowded low-vol consensus about to break — (3) CFTC CoT net length / fund-flow / options skew, (4) the pain trade. Never use an averaged/mean-reversion RSI for this.

**Technical analysis and RSI COUNT in the score.** They are not just displayed — fold them into the live conviction shown on the badge: trade ideas/live book = core 10 + TA (0–2) + RSI (0–1) = **/13**; Derivatives Desk = base 8 + TA + RSI = **/11**. Recompute every refresh from `fetch_rsi` technicals.
...
- **Summary (index):** the overnight read (last 24h, global), takeaways, Bull/Base/Bear, what the tape is missing — then **"So what for the book"**, the macro→client seam (see below).
  - **"So what for the book"** closes the Summary and is the personalisation moment — today's tape read onto the *live Fable book*, NOT a repeat of the Portfolio page's static actions. Author it fresh each refresh into `brief["book_outlook"]`: `{commentary, outperform:[{name,why}], underperform:[{name,why}], watch:[{label,text}]}`. **commentary** = what today's news actually does to *this* book (name the real holdings and their two-sided exposure to the day's stories). **outperform / underperform** = which of the book's actual positions you expect to lead and lag *today*, each with a one-line *why* tied to today's tape (not generic). **watch** = what else to weigh today — hedging the concentration, FX into the next event, rolling bonds, adds (software/power not more semis). Specific to today; if you wouldn't say it differently tomorrow, it's too generic. Holdings live in `client-book-json-Aurora.json`; `book_outlook` renders only when a scan is present.
...
- **Derivatives Desk** (NOT "Fable structuring desk" — just "Derivatives Desk"): headings; ideas grouped by asset class with each tagged by **product type + objective**, respecting the shelf — *equities* get the full income/participation shelf (RevCon, FCN, Phoenix, digital review, autocall Market Plus, BREN, protected participation) PLUS **equity OTC with full creative latitude** (accumulators/decumulators, call/put spreads, risk reversals, prepaid variable forwards, total-return swaps, worst-of/dispersion, variance/vol — whatever genuinely fits the book); *FX* gets DCS (income) and forwards/seagulls/puts/accumulators (hedging); *rates* get range accruals only. **This shelf is a GUIDELINE, not a hard limit** — it is what trades cleanly today. If an idea genuinely needs a structure outside it (or off-piste OTC — where essentially anything is possible), still suggest it and flag "please check with your derivs team on feasibility of trading." Never hold back a good idea because it is off-shelf; just tag the feasibility check. Same TA + absolute-RSI + positioning as the other tabs. A **Recovering losses** section gives the exact route for each underwater position and, where it fits, a faster structured-product + OTC-at-margin path.
