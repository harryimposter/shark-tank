...
def _fallback_mode():
    """True when this render is a data-only fallback (GitHub Action), not a full web-sourced
    refresh. Set MARKETMAP_REFRESH_MODE=fallback in the CI environment. Returns (on, from_date)."""
    on = os.environ.get("MARKETMAP_REFRESH_MODE", "").strip().lower() == "fallback"
    return on, os.environ.get("MARKETMAP_FALLBACK_FROM", "").strip()
...
def _coverage_bar():
    ...
    fb, from_date = _fallback_mode()
    narr = ('<span style="color:var(--gold);font-weight:600">macro narrative carried over'
            + (f' from {e(from_date)}' if from_date else '')
            + ' — data-only fallback refresh, no fresh web sweep</span>') if fb else \
...
def _fallback_banner():
    """Prominent amber notice for the Summary page when running in data-only fallback mode."""
    fb, from_date = _fallback_mode()
    if not fb:
        return ""
    now = datetime.now()
    src = f' from {e(from_date)}' if from_date else ''
    return (
        '<div style="border:.5px solid var(--gold);border-left:3px solid var(--gold);'
        'border-radius:var(--rad-lg);background:rgba(184,150,12,.07);padding:.8rem 1.05rem;'
        'margin:0 0 1.2rem;font-size:12.5px;line-height:1.6;color:var(--ink)">'
        '<b style="text-transform:uppercase;letter-spacing:.08em;font-size:11px;color:var(--gold)">'
        'Data-only fallback refresh</b><br>'
        'The scheduled full refresh did not run, so this brief was regenerated by the automated '
        'fallback: <b>live marks, RSI, technicals, the screener and the book are fresh</b> as of '
        f'{now:%a %d %b %H:%M} local, but the <b>macro narrative, scenarios and talking points are '
        f'carried over{src}</b> — they were not re-sourced from the web for this window. Read the '
        'numbers as live and the prose as context pending the next full refresh.'
        '</div>'
    )
...
def _universe_block(screen):
    ...
    return (
        ...
        'Japan, Korea, China.</li>'
        '<li><b>Cross-asset, not just stocks.</b> Single names across every sector <i>plus</i> liquid ETFs spanning '
        'equity, small-cap, <b>rates (TLT)</b>, <b>credit (HYG)</b>, <b>gold/silver</b> and EM/EAFE — so the screen '
        'flags extremes in bonds, credit and commodities too, not only equities.</li>'
        '<li><b>Thematic overlay:</b> an explicit <b>AI build-out sleeve</b> — semis, semicap, optical, '
        'connectivity, AI power/cooling, AI servers — because that is the dominant alpha theme right now.</li>'
        '<li><b>Excluded:</b> micro-caps, illiquid / niche single names, leveraged or inverse ETPs, and anything '
        'without a clean 6-month daily history.</li>'
        '</ul>'
